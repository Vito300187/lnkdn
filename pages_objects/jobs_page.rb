  ...
  class Search
    class Jobs
      ...
      def change_filter_jobs_to(item)
        ...
        content_index = all(:xpath, @select_search_filter_list).index { |i| i.text == item }
    ...
    class People
      ...
      def connect_with_hr(add_a_note=nil)
        all(:xpath, @make_a_contact).each do |button_connect|
          ...
          WaitUtil.wait_for_condition(
            'Connect name change to Pending',
            timeout_sec: 10,
            delay_sec: 1
          # ) { button_connect.text.eql?('Pending') }
          ) { !button_connect.text.eql?(@connect_button_name) }
        end
      end
